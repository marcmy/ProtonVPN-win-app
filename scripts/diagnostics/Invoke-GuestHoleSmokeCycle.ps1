...
try
{
    ...
    if ($status -eq 'Failed')
    {
        throw 'GuestHoleManager did not complete the requested Guest Hole disconnect. The disconnected-state capture was not started. This smoke cycle is over, so it is now safe to reconnect manually for recovery; do not use that reconnect-induced disconnect as phase-C evidence.'
    }
